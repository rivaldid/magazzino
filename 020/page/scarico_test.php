...
if ($DEBUG) {
	if (isset($destinazione)) $log .= remesg("Valore destinazione: ".$quantita,"debug");
	...
	if ($valid) {
		...
		if ($ritorno[0]=="0") {
			...
			if (!(isset($_SESSION['mds'])) OR empty($_SESSION['mds'])) {
				...
				if ($DEBUG) $log .= remesg("Testa MDS: ".$corpo_html,"debug");
				if ($DEBUG) $log .= remesg("MDS fin'ora: ".$_SESSION['mds'],"debug");
			...
			if ($DEBUG) $log .= remesg("Corpo MDS: ".$corpo_html,"debug");
			if ($DEBUG) $log .= remesg("MDS fin'ora: ".$_SESSION['mds'],"debug");
			...
			if(isset($_SESSION['save'])) {
				...
				if ($DEBUG) $log .= remesg("Coda MDS: ".$corpo_html,"debug");
				if ($DEBUG) $log .= remesg("MDS fin'ora: ".$_SESSION['mds'],"debug");
				...
				// 222553. reset richiedente - data_doc_scarico - note
				unset($richiedente,$data_doc_scarico,$note);
				unset($_SESSION['richiedente'],$_SESSION['data_doc_scarico'],$_SESSION['note']);

				// 222554. reset sessione server
				reset_sessione();
